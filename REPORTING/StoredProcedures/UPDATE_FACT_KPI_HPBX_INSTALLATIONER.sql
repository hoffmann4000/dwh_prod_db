CREATE PROCEDURE [REPORTING].[UPDATE_FACT_KPI_HPBX_INSTALLATIONER]
AS 
BEGIN
IF OBJECT_ID('REPORTING.FACT_KPI', 'U') IS NOT NULL 

PRINT 'OPDATERER KPI "HOSTED PBX INSTALLATIONER" I REPORTING.FACT_KPI';

MERGE REPORTING.FACT_KPI T
	USING 
	(
	SELECT 
	SUM(PBX) AS REALISERET,
	EXTERNAL_PARTNER_ID AS FORHANDLER_ID,
	DATO AS DATE_ID,
	(SELECT KPI_ID FROM REPORTING.DIM_KPI WHERE KPI_NAVN='HostedPBX Installationer ') AS KPI_ID	
	...
	--DER SUMMERES OG GRUPPERES FORDI EN FORHANDLER KAN HAVE KUNDER I FLERE ONE CONNECT DB
