...
SELECT 
COUNT(DISTINCT V.ABN_ID) AS ABONNOMENTER, YEAR(V.DATE_ID) AS ÅR, MONTH(V.DATE_ID) AS MÅNED,
COUNT(DISTINCT V.ABN_ID)-LAG (COUNT(DISTINCT V.ABN_ID)) OVER (PARTITION BY V.ABN_NAVN ORDER BY YEAR(DATE_ID), MONTH(V.DATE_ID) ASC) AS NETTO_VAEKST
...
AND DATE_ID<=GETDATE()
GROUP BY V.ABN_NAVN, YEAR(DATE_ID), MONTH(DATE_ID);
