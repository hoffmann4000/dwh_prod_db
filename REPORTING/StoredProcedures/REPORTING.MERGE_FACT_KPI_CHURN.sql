ALTER PROCEDURE [REPORTING].[MERGE_FACT_KPI_CHURN]
AS 
BEGIN
...
MERGE REPORTING.FACT_KPI T
	USING 
	(
	SELECT CHURN_PCT AS REALISERET, PARTNER_ID AS FORHANDLER_ID, DATE_ID, 78 AS KPI_ID
	FROM REPORTING.FACT_CHURN
	WHERE DATE_ID IN (SELECT LASTDAYOFMONTH FROM BASELINE.LASTDAYOFMONTH_ONE_YEAR_BACK)
	) S
