CREATE PROCEDURE [BASELINE].[CREATE_DIM_DATO]
AS

DROP TABLE IF EXISTS [BASELINE].[DIM_DATO];

CREATE TABLE [BASELINE].[DIM_DATO](
...
/* Populate Your Dimension Table with values*/
	...
	SELECT
		...
		CASE DATEPART(DW, @CurrentDate)
			...
			WHEN 7 THEN 0
			END AS IsWeekday,
		NULL AS HolidayUSA, Null, Null, Null, Null, Null, Null
...
--Step 3.
/*Opdaterer værdier i kolonnen HolidayDK. Faste danske helligdage tilføjet i koden. Koden tager ikke højde for påsken. */
	
-- 1. nytårsdag  
	UPDATE [BASELINE].[DIM_DATO]
		SET HolidayDK = '1. nytårsdag'
	WHERE [Month] = 1 AND [DayOfMonth]  = 1

-- Store bededag
	UPDATE [BASELINE].[DIM_DATO]
		SET HolidayDK = 'Store bededag'
	WHERE [Month] = 4 AND [DayOfMonth]  = 30

-- Kristi Himmelfartsdag 
   UPDATE [BASELINE].[DIM_DATO]
		SET HolidayDK = 'Kristi Himmelfartsdag'
	WHERE [Month] = 5 AND [DayOfMonth]  = 13

-- 1. juledag 
   UPDATE [BASELINE].[DIM_DATO]
		SET HolidayDK = '1. Juledag'
	WHERE [Month] = 12 AND [DayOfMonth]  = 25

--2. juledag
   UPDATE [BASELINE].[DIM_DATO]
		SET HolidayDK = '2. Juledag'
	WHERE [Month] = 12 AND [DayOfMonth]  = 26

--Update flag for DK Holidays 1= Holiday, 0=No Holiday
	
	UPDATE [BASELINE].[DIM_DATO]
		SET IsHolidayDK  = CASE WHEN HolidayDK   IS NULL 
		THEN 0 WHEN HolidayDK   IS NOT NULL THEN 1 END
		
--Finansår (numerisk) beregnes
	UPDATE [BASELINE].[DIM_DATO]
		SET FiscalYear =		case 
	...
	UPDATE [BASELINE].[DIM_DATO]
		SET FiscalYearText = case 
							when month(Date) between 1 and 6  then concat('Regnskabsår',year(Date)-1,'-',year(Date))
							when month(Date) between 7 and 12  then concat('Regnskabsår',year(Date),'-',year(Date)+1)
							end

	UPDATE [BASELINE].[DIM_DATO]
		SET FiscalYearStart = cast 
								(
								case 
								when month between 1 and 6 then concat (year-1,'-','07','-','01')
								when month between 7 and 12 then concat (year,'-','07','-','01') 
								else null 
								end AS DATE
								)
	...
	UPDATE [BASELINE].[DIM_DATO]
		SET FiscalYearEnd = cast 
								(
								case 
								when month between 1 and 6 then concat (year,'-','06','-','30')
								when month between 7 and 12 then concat (year+1,'-','06','-','30') 
								else null 
								end AS DATE 
								)
